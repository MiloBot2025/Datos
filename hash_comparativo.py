    ...
    steps:
      ...
      - name: Run script
        ...
        run: |
          ...
          rsync -av "$WORKDIR/_reports/" "$WORKDIR/site/public_reports/" || true
          rsync -av "$WORKDIR/public_listas/" "$WORKDIR/site/public_listas/" || true
          python - <<'PY'
import pathlib, datetime, os
base = pathlib.Path(os.environ["WORKDIR"])
pub  = base / "site"
(pub / "public_reports").mkdir(parents=True, exist_ok=True)
(pub / "public_listas").mkdir(parents=True, exist_ok=True)

def links(sub, title):
    ul = []
    for p in sorted((pub/sub).glob("*")):
        if p.is_file():
            size_kb = p.stat().st_size // 1024
            ul.append(f'<li><a href="{sub}/{p.name}">{p.name}</a> — {size_kb} KB</li>')
    if not ul: ul.append("<li>(vacío)</li>")
    return f"<h2>{title}</h2><ul>{''.join(ul)}</ul>"
